// Jono forked from: https://github.com/ateliee/jquery.schedule
...
(function ($) {
  ...
  var methods = {
    ...
    formatTime: function formatTime(val) {
      ...
      // var tensHour = Math.floor(val / 36000)
      // if
      var h =
        '' + (Math.floor(val / 36000) || '') + Math.floor((val / 3600) % 10)
    ...
    // Jonos method
    findItem: function findItem(entryId) {

      var $this = $(this);
      var rows = methods.timelineData.apply($this);

      for (var r in rows) {
        var row = rows[r];

        for (var i in row.schedule) {
          var item = row.schedule[i]
          if (item.data.entry == entryId) {
            return [r, i]
          }
        }
      }
    ...
    // Jonos method
    updateItem: function updateItem(entryId, updatedItem) {

      return this.each(function () {
        var $this = $(this)

        var index = methods.findItem.apply($this, [entryId])
        var rows = methods.timelineData.apply($this)

        rows[index[0]].schedule[index[1]] = updatedItem
        methods.setRows.apply($this, [rows])
      });
    },

    // Jonos method
    deleteItem: function deleteItem(entryId) {
      ...
      return this.each(function () {
        ...
        var index = methods.findItem.apply($this, [entryId])
        var rows = methods.timelineData.apply($this)
        delete rows[index[0]].schedule[index[1]]
    ...
    _addScheduleData: function _addScheduleData(timeline, d) {
      ...
      return this.each(function () {
        ...
        // $bar.find('.text').html('<i class="bi-info-circle"></i>');
        ...
        icon = '<i class="bi-car-front-fill"></i>'
        if (data.data.class === 'passenger') {
          var icon = '<i class="bi-person-fill"></i>'
        }

        $bar.find('.text').html(icon)
