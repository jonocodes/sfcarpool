...
--     OR
-- UPDATE
--     ON public.events for each ROW EXECUTE FUNCTION audit.if_modified_func('true');
-- CREATE TRIGGER audit_trigger_stm after TRUNCATE
--     ON public.events for each statement EXECUTE FUNCTION audit.if_modified_func('true');
-- CREATE TRIGGER set_public_events_updated_at before
-- UPDATE
--     ON public.events for each ROW EXECUTE FUNCTION set_current_timestamp_updated_at();
